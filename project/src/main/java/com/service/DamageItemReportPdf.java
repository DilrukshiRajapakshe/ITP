...
public class DamageItemReportPdf extends AbstractPdfView{
	@Override
	protected void buildPdfDocument(Map model, Document document,PdfWriter writer, HttpServletRequest request,HttpServletResponse response) throws Exception {
		...
		response.setHeader("Content-Disposition", "attachment; filename=\"DamageStockReport.pdf\""); //This code responsible for direct download PDF file
		...
		ArrayList list = (ArrayList) model.get("result");
          ...
          f.setSize(8);
          Paragraph p1 = new Paragraph("Damage Item Report", f1);
          ...
          while (itr.hasNext()) {          
	           ...
	           String amount = (String) listView.get(5);
	           String type = (String) listView.get(6);
	           String cost = (String) listView.get(7);
