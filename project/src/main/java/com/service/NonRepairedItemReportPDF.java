...
public class NonRepairedItemReportPDF extends AbstractPdfView{
	@Override
	protected void buildPdfDocument(Map model, Document document,PdfWriter writer, HttpServletRequest request,HttpServletResponse response) throws Exception {
		...
		response.setHeader("Content-Disposition", "attachment; filename=\"NonRepairedItemReport.pdf\""); //This code responsible for direct download PDF file
		...
		ArrayList list = (ArrayList) model.get("result");
          ...
          f.setSize(8);
          Paragraph p1 = new Paragraph("Non Repaired Item Report", f1);
