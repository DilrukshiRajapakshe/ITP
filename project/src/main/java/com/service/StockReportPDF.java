...
public class StockReportPDF extends AbstractPdfView{
	...
	protected void buildPdfDocument(Map model, Document document,PdfWriter writer, HttpServletRequest request,HttpServletResponse response) throws Exception {
		...
		response.setHeader("Content-Disposition", "attachment; filename=\"StockReport.pdf\""); //This code responsible for direct download PDF file
		...
		ArrayList list = (ArrayList) model.get("result");
          ...
          f.setSize(8);
          Paragraph p1 = new Paragraph("Stock Report", f1);
